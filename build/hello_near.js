...
let DonacionesContract = (_dec = NearBindgen({}), _dec2 = view(), _dec3 = call({}), _dec4 = view(), _dec(_class = (_class2 = class DonacionesContract {
  ...
  agregar_donacion({
    ...
    const remitente = signerAccountId(); // Obtiene la cuenta del remitente
    log(`Agregando donación de ${remitente} por un monto de ${monto}`);
    ...
    log(`Donación agregada exitosamente`);
  }

  // Método para obtener una donación específica por el índice
  get_donacion_por_indice({
    indice
  }) {
...
}, _applyDecoratedDescriptor(_class2.prototype, "get_donaciones", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "get_donaciones"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "agregar_donacion", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "agregar_donacion"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "get_donacion_por_indice", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "get_donacion_por_indice"), _class2.prototype), _class2)) || _class);
function get_donacion_por_indice() {
  const _state = DonacionesContract._getState();
  if (!_state && DonacionesContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = DonacionesContract._create();
  if (_state) {
    DonacionesContract._reconstruct(_contract, _state);
  }
  const _args = DonacionesContract._getArgs();
  const _result = _contract.get_donacion_por_indice(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DonacionesContract._serialize(_result, true));
}
function agregar_donacion() {
  ...
  const _args = DonacionesContract._getArgs();
  const _result = _contract.agregar_donacion(_args);
  DonacionesContract._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DonacionesContract._serialize(_result, true));
}
function get_donaciones() {
  const _state = DonacionesContract._getState();
  if (!_state && DonacionesContract._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  ...
  const _args = DonacionesContract._getArgs();
  const _result = _contract.get_donaciones(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(DonacionesContract._serialize(_result, true));
}

export { agregar_donacion, get_donacion_por_indice, get_donaciones };
